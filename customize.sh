...
# Extract MagicDNS forwarder (smartdns) runtime libraries for current arch
mkdir -p "$TS_DIR/lib"
unzip -qqjo "$ZIPFILE" "tailscale/lib/$ARCH/*" -d "$TS_DIR/lib" 2>/dev/null || true
...
set_perm_recursive "$TS_DIR/lib/" 0 0 0755 0644 "u:object_r:system_file:s0"
# libc.so doubles as the musl dynamic loader (smartdns is started by
# exec'ing it), so it must be executable, unlike the other libs.
set_perm "$TS_DIR/lib/libc.so" 0 0 0755 "u:object_r:system_file:s0"
