...
# ---- MagicDNS conditional forwarder (SmartDNS) ----
# TS_DNS_ENABLE=1 turns on local conditional DNS forwarding:
#   queries for *.ts.net are answered DIRECTLY from live tailscale status
#   (SmartDNS domain-rules -address 100.x, no tailscaled DNS server); all
#   other queries are forwarded to fixed public DNS 223.5.5.5/119.29.29.29
#   (or TS_DNS_UPSTREAM if set).
export TS_DNS_ENABLE="${TS_DNS_ENABLE:-1}"
export TS_DNS_PORT="${TS_DNS_PORT:-15353}"                      # local listen port (15353: avoid mDNS 5353)
export TS_MAGICDNS_SERVER="${TS_MAGICDNS_SERVER:-100.100.100.100}" # tailscaled DNS proxy
export TS_MAGICDNS_DOMAINS="${TS_MAGICDNS_DOMAINS:-ts.net}"     # space-separated suffixes
export TS_DNS_UPSTREAM="${TS_DNS_UPSTREAM:-}"                   # e.g. "223.5.5.5 119.29.29.29"; empty = fixed public DNS
...
# Logs function
log() {
  # Timestamp is refreshed per call: the watchdog is a long-lived process and
  # a startup-time value would stamp every later line with the boot minute.
  CURRENT_TIME=$(date +"%I:%M %P")
